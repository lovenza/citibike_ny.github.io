...
ride_analysis_data <- bike_weather %>%
  mutate(
    ...
    temp_category = case_when(
      tmax >= 80 ~ "Warm (≥80°F)",
      tmax >= 70 ~ "Mild (70-79°F)",
      TRUE ~ "Cool (<70°F)"
    ),
    temp_category = factor(temp_category, levels = c("Cool (<70°F)", "Mild (70-79°F)", "Warm (≥80°F)")),
...
We employ multiple linear regression to model individual ride duration as a function of weather and temporal predictors. The large sample size (N ≈ 100,000) provides excellent statistical power.
...
model_comparison <- bind_rows(
  tidy(model_total_full) %>% mutate(model = "All Riders"),
  tidy(model_member_full) %>% mutate(model = "Members"),
  tidy(model_casual_full) %>% mutate(model = "Casual")
) %>%
  filter(!str_detect(term, "time_of_day")) %>%  # Exclude time variables for space
...
model_comparison %>%
  filter(term %in% c("tmax", "tmin", "prcp", "distance_km", "is_saturdayTRUE", "is_sundayTRUE")) %>%
  mutate(
    term = case_when(
      term == "tmax" ~ "Max Temperature (°F)",
      term == "tmin" ~ "Min Temperature (°F)",
      term == "prcp" ~ "Precipitation (inches)",
      term == "distance_km" ~ "Distance (km)",
      term == "is_saturdayTRUE" ~ "Saturday",
      term == "is_sundayTRUE" ~ "Sunday",
      TRUE ~ term
    )
  ) %>%
  kable(
    caption = "Table 3: Regression Coefficients (Effect on Ride Duration in Minutes)",
    digits = 3,
    col.names = c("Model", "Predictor", "Estimate", "SE", "t-stat", "p-value", "Sig.")
  ) %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE) %>%
  pack_rows("All Riders", 1, 6) %>%
  pack_rows("Members Only", 7, 12) %>%
  pack_rows("Casual Users Only", 13, 18)
```
...
performance_metrics <- tibble(
  ...
  N = c(nrow(ride_analysis_data), nrow(ride_member), nrow(ride_casual)),
  R_squared = c(
  ...
  F_statistic = c(
    summary(model_total_full)$fstatistic[1],
    summary(model_member_full)$fstatistic[1],
    summary(model_casual_full)$fstatistic[1]
  )
)

performance_metrics %>%
  kable(
    caption = "Table 4: Model Performance Metrics",
    digits = 3,
    col.names = c("Model", "N", "R²", "Adj R²", "RMSE (min)", "F-statistic")
  ) %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE)
```
...
coef_total <- coef(model_total_full)
coef_member <- coef(model_member_full)
coef_casual <- coef(model_casual_full)
...
temp_effect_10F_total <- coef_total["tmax"] * 10
temp_effect_10F_member <- coef_member["tmax"] * 10
temp_effect_10F_casual <- coef_casual["tmax"] * 10

prcp_effect_total <- coef_total["prcp"]
prcp_effect_member <- coef_member["prcp"]
prcp_effect_casual <- coef_casual["prcp"]

distance_effect_total <- coef_total["distance_km"]
distance_effect_member <- coef_member["distance_km"]
distance_effect_casual <- coef_casual["distance_km"]
```

## KEY FINDING 1: Temperature Modestly Affects Ride Duration

**Effect of 10°F Temperature Increase:**

```{r temp_findings_duration}
temp_findings_duration <- tibble(
  `User Type` = c("All Riders", "Members", "Casual"),
  `Effect (minutes)` = c(
    sprintf("%+.2f", temp_effect_10F_total),
    sprintf("%+.2f", temp_effect_10F_member),
    sprintf("%+.2f", temp_effect_10F_casual)
  ),
  `Percent of Mean` = c(
...
temp_findings_duration %>%
  kable(
    caption = "Table 5: Effect of 10°F Temperature Increase on Ride Duration",
    align = c('l', 'r', 'r', 'c')
  ) %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE)
```
...
**Temperature has a `r ifelse(temp_effect_10F_total > 0, "positive", "negative")` but modest effect on ride duration.** For every 10°F increase in maximum temperature, rides are **`r sprintf("%.2f", abs(temp_effect_10F_total))` minutes `r ifelse(temp_effect_10F_total > 0, "longer", "shorter")`** on average (`r sprintf("%+.1f%%", (temp_effect_10F_total/mean_duration_total)*100)` change). This effect is `r ifelse(pval_total["tmax"] < 0.001, "highly significant (p < 0.001)", "significant")` with our large sample of `r format(nrow(ride_analysis_data), big.mark=",")` rides.

**User Comparison:** `r ifelse(abs(temp_effect_10F_casual) > abs(temp_effect_10F_member), "Casual riders", "Members")` show slightly larger temperature effects, but the differences are small for both groups.
...
**Distance is by far the strongest predictor of ride duration.** Each additional kilometer traveled adds approximately **`r sprintf("%.1f", distance_effect_total)` minutes** to ride duration. This makes intuitive sense: longer distances require more time. Weather and temporal effects are small in comparison.
...
**Riders take slightly shorter trips when it rains.** Each inch of precipitation is associated with rides that are **`r sprintf("%.2f", abs(prcp_effect_total))` minutes shorter**. This suggests riders minimize their time in the rain by taking more direct routes or shortening discretionary trips.
...
**Casual riders take significantly longer trips than members.** On average, casual rides are **`r sprintf("%.1f", mean_duration_casual - mean_duration_member)` minutes longer** (`r sprintf("%.0f%%", ((mean_duration_casual/mean_duration_member)-1)*100)` increase). This reflects different usage patterns:
...
- Temperature: +0.1-0.2 minutes per 10°F (significant but small)
...
- Casual riders: ~`r sprintf("%.0f%%", ((mean_duration_casual/mean_duration_member)-1)*100)` longer trips
