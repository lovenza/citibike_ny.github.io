...
<div class="hero-section" style="text-align: center; padding: 40px 20px; background-color: #f9f9f9; border-radius: 8px;">
  <h1 style="margin-bottom: 10px;">📊 Final Project Report</h1>
  <p style="font-size: 1.2em; margin-bottom: 5px;"><strong>Analyzing NYC Citi Bike Ridership Patterns in Response to Daily Rhythms and Weather</strong></p>
  <p style="color: #555;">Columbia University Mailman School of Public Health | Data Science I (P8105) | Fall 2025</p>
</div>

<div class="image-container" style="text-align: center; margin-top: 40px; margin-bottom: 20px;">
  <img src="pictures/pic1.png" alt="Citi Bike Lifestyle" style="width: 90%; max-width: 800px; border-radius: 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
  <p style="font-size: 0.9em; color: #666; font-style: italic; margin-top: 10px;">Citi Bike brings vibrancy to NYC streets</p>
</div>
...
Understanding how people use Citi Bike—and what factors influence their decisions to ride—has important implications across multiple domains:
...
3. **Weather Impact:** To what extent does daily weather—temperature and precipitation—influence the decision to ride a bike? Can we quantify these effects?
...
weather_clean <- weather_data %>%
  mutate(
    tmax = (tmax / 10) * 9/5 + 32,  # Convert from tenths of °C to °F
    tmin = (tmin / 10) * 9/5 + 32,
...
**Validation Checks:**
- All durations between 0 and 120 minutes ✓
- All distances between 0 and 50 km ✓
- All dates within September 2025 ✓
- Member/casual classifications valid ✓
- Temperature values reasonable for September in NYC (50-85°F) ✓
...
- **Rows:** 30 days × 2 user types = 60 observations
- **Columns:** 8 variables  
...
<div style="text-align: center; margin: 20px 0;">
  <img src="pictures/pic2.png" alt="User with Citi Bike" style="width: 80%; border-radius: 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
  <p style="font-size: 0.9em; color: #666; font-style: italic;">Exploring user preferences: Classic vs. Electric Bikes</p>
</div>
...
<div style="text-align: center; margin: 20px 0;">
  <img src="pictures/pic3.png" alt="NYC Citi Bike Station Map" style="width: 100%; border-radius: 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
  <p style="font-size: 0.9em; color: #666; font-style: italic;">Spatial distribution of Citi Bike rides in NYC</p>
</div>
...
1. **Strong Positive Correlation:**
   ...
   - Relationship appears roughly linear in 60-85°F range

2. **Optimal Temperature Range:**
   - Peak ridership: 70-80°F
   - Below 60°F: Noticeable decline
   - Above 85°F: Slight decline (too hot?)
...
2. **Weather "Threshold" for Cycling:**
   - Below 55°F + Rain: Very low ridership
   - Above 70°F + Dry: Very high ridership
...
- **Unit of analysis:** Day × User Type
- **Sample size:** 60 observations (30 days × 2 user types)
- **Outcome variable:** Total rides per day
- **Predictors:** Temperature (°F), Precipitation (inches), Day of week
...
- Temperature: ρ = 0.66 (similar to Pearson)
- Precipitation: ρ = -0.62 (stronger than Pearson)
...
1. **Temperature Effect:** Each 10°F increase in temperature associated with ~684 additional rides per day (p < 0.001)
   - Going from 60°F to 80°F: ~1,368 more rides
...
1. **Temperature Sensitivity:** +523 rides per 10°F increase
...
1. **High Temperature Sensitivity:** +161 rides per 10°F increase
...
# RMSE: 428 rides
# MAE: 345 rides
# R²: 0.68 (slightly lower than training R²)
```
...
1. **Operational Planning:** A 20°F temperature change could swing daily ridership by ~1,400 rides (20% of typical volume), requiring adjusted bike distribution.
...
Model | R² | Adj. R² | RMSE | Purpose
------|-----|---------|------|--------
Total | 0.72 | 0.69 | 485 | Overall planning
Member | 0.68 | 0.64 | 392 | Commuter focus
Casual | 0.75 | 0.72 | 93 | Tourist operations

**Interpretation:** The casual rider model actually fits best (highest R²), suggesting their behavior is most predictable from weather and temporal factors. Member behavior has more unexplained variation, possibly due to factors we don't measure (e.g., remote work days, individual preferences).
...
**Temperature Effects:**
- Each 10°F increase → ~684 additional daily rides
- Effect strongest for casual riders (+161 per 10°F)
- Optimal range: 70-80°F
- Below 60°F: Noticeable ridership decline

**Precipitation Effects:**
- Each inch of rain → ~1,840 fewer daily rides  
- Even light rain (<0.1 in) → 10-15% decline
- Heavy rain (>0.5 in) → 30-40% decline
...
1. **Magnitude of Weather Effects:** The 72% R² value was higher than expected, suggesting weather is even more important than anticipated.
...
4. **Member Rain Resilience:** While members were less rain-sensitive, they still showed a 25-30% decline in heavy rain—more than we expected for "committed" commuters.
...
1. **Demand Forecasting:** The predictive models (R² = 0.72) enable accurate daily demand forecasting based on weather forecasts, improving bike redistribution and dock management.
...
**Observational Data:**
- Cannot definitively establish causation (weather → ridership)
...
4. **Pricing Experiments:** How does pricing affect usage patterns and equity?

5. **E-Bike Effects:** Do electric bikes change who rides and how far they go?
...
Weather—particularly temperature and precipitation—exerts strong, quantifiable effects on ridership, explaining 72% of day-to-day variation. These effects are actionable: operators can use weather forecasts to predict demand, adjust bike distribution, and optimize operations.

The findings have implications beyond bike-share operations. They inform urban planning decisions about bike infrastructure investment, public health initiatives to promote active transportation, and environmental sustainability efforts to reduce car dependence. Understanding what drives people to choose cycling—and what barriers prevent them—is essential for creating more livable, sustainable cities.
