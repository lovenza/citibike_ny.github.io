...
```{r correlation_tests, include=FALSE}
...
**Temperature vs Duration:** Temperature shows a negligible correlation with ride duration (r = 0.006, p < 0.001), indicating that while statistically significant due to large sample size, the practical effect is minimal.

**Precipitation vs Duration:** Precipitation has essentially no correlation with ride duration (r = 0.001, p = 0.805), suggesting that rain affects the decision to ride but not trip length once riders commit.

**Distance vs Duration:** Distance demonstrates a strong positive correlation with ride duration (r = 0.651, p < 0.001), confirming it is the dominant predictor of how long rides take.
...
# Temperature effect (per 10°F increase)
...
Temperature has a positive but modest effect on ride duration. For every 10°F increase in maximum temperature, rides are approximately 0.1-0.2 minutes longer on average. This effect is statistically significant with our large sample but practically small.
